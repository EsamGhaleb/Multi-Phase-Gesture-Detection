...
class Feeder(Dataset):
    def __init__(self, data_path, label_path,
                 ...
                 window_size=-1, normalization=False, debug=False, use_mmap=True, random_mirror=False, random_mirror_p=0.5, is_vector=False, fold=0):
        """
    ...
    def load_data(self):
        ...
        try:
            with open(self.label_path) as f:
                self.sample_name, self.pair_speaker_referent, self.label = pickle.load(f)
        except:
            # for pickle file from python2
            with open(self.label_path, 'rb') as f:
                self.sample_name, self.pair_speaker_referent, self.label = pickle.load(f, encoding='latin1')
        ...
        if self.debug:
            ...
            self.pair_speaker_referent = self.pair_speaker_referent[0:100]
    ...
    def __getitem__(self, index):
        ...
        speaker = '_'.join(self.pair_speaker_referent[index].split('_')[0:2])
        referent = self.pair_speaker_referent[index].split('_')[2]
        start_frame = self.sample_name[index].split('_')[2]
        end_frame = self.sample_name[index].split('_')[3]
        # make a dictionary to store the data
        labels_dict = {'label': label, 'actual_label': actual_label, 'speaker': speaker, 'referent': referent, 'start_frame': int(start_frame), 'end_frame': int(end_frame), 'keypoints': data_numpy, 'fold': self.fold}
        ...
        if self.random_mirror:
            if random.random() > self.random_mirror_p:
                if data_numpy.shape[2] == 49:
                    flip_index = mediapipe_flip_index
                if data_numpy.shape[2] == 27:
        ...
        return data_numpy, label, index, labels_dict
